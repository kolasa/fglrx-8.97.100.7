...
static int __init firegl_init_module(void)
{
    ...
    dev->pubdev.psigmask = (kcl_sigset_t*)&dev->sigmask;
...
void ATI_API_CALL KCL_SIGNAL_ConfigureMask(kcl_sigset_t *pSigMask)
{
...
int KCL_SIGNAL_Notifier(void *priv)
{
 ...
 */
int ATI_API_CALL KCL_TestAndClearPageDirtyFlag(unsigned long virtual_addr)
{
    ...
    if (PMD_HUGE(*pmd_p))
    {
        KCL_DEBUG1(FN_FIREGL_KCL,"Trying to clear dirty bits of huge page\n");
        return -1;
    }

    pte_p = PTE_OFFSET_FUNC(vma->vm_mm, pmd_p, virtual_addr, &ptl);

    if (pte_present(*pte_p))
    {
        ret = (ptep_test_clear_dirty(vma, virtual_addr, pte_p) ? 1 : 0);
    }
    else
    {
    ...
    KCL_DEBUG2(FN_FIREGL_KCL,"vma:0x%lx, return %d\n", (long)vma, ret);
    return ret;
}
